...
Tested on `dev-app-api-cl01` (us-east-1, 2× t3.medium nodes). Postgres is bundled — no RDS needed for this setup.

**Prerequisites:**
- EKS cluster running with `kubectl` configured
- nginx ingress controller installed
- Docker + AWS CLI available
- ECR Public access (no credentials needed for pulls)

### 1. Build and push the image

Run from inside `phoenix_live_view_example/` (the app source):

```bash
git clone https://github.com/chrismccord/phoenix_live_view_example
cd phoenix_live_view_example
cp /path/to/limitless-devops/runtime.exs .
cp /path/to/limitless-devops/page_live.ex lib/demo_web/live/page_live.ex
...
docker build \
  -f /path/to/limitless-devops/Dockerfile \
...
> **Note:** If building on Apple Silicon (arm64), build inside AWS CloudShell (native linux/amd64) to avoid a platform mismatch.
...
1. Deploys Postgres (bundled, no RDS needed)
2. Waits for Postgres to be ready (`pg_isready` loop)
3. Creates the database, runs migrations, seeds 1000 users
4. Starts the Phoenix app
...
Add a CNAME in your DNS provider: `phoenix.autometalabs.io` → ELB hostname.

### 5. Verify

```bash
kubectl get pods                          # both pods should be 1/1 Running
kubectl logs deployment/phoenix-demo-phoenix-liveview-demo --tail=20
curl -H "Host: phoenix.autometalabs.io" http://<ELB-hostname>
```
